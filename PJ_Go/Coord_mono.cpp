...
void Coord_mono::printstr(const int x, const int y, const string &str, int color )
{
    if(color == 1)
    {
    ...
    {
        printchar(x + 8 * i, y, str[i], color);
    }
}

void Coord_mono::printchar(const int x, const int y, const char c, int color )
{
    ...
    const int y_pos = (flag_coord == world_coord ? height - y -1: y-1); //两种坐标系
...
void Coord_mono::line(const int x0, const int y0, const int x1, const int y1, int color )
{
...
void Coord_mono::bit_change(const int x, const int y, int color) //两种存储方式的转化
{
    const int x_pos = (x - 1) / 8; // canvas中的真实位置
    ...
    const int right = (x - 1) % 8;                                                         // 右移的位数
    const int left = 8 - right;                                                            // 左移的位数
    dot_canvas[y_pos][x_pos] = dot_canvas[y_pos][x_pos] | (unsigned char)pow(2, left - 1) * color; //用位运算直接转化
}

void Coord_mono::rect(int x1, int y1, int width, int height, int color )
{
...
void Coord_mono::showViewAt(int x1, int y1, View* V)
{
    const int y_pos = (flag_coord == world_coord ? height - y1 : y1); //两种坐标系
    const int x_pos = (x1 - 1) / 8; // canvas中的真实位置
    // const int y_pos = p_view->height - y;      // canvas中的真实位置
    const int right = (x1 - 1) % 8;               //右移的位数
    const int left = 8 - right;                  // 左移的位数
    for (int i = 0; i < V->get()->height; i++) // 对一个char的8个二进制数进行循环（字符表从上到下）（char从下到上）
    {
        ...
        for (int j = 0; j < V->get()->width; j++)
        {
            ...
            dot_canvas[y_pos - i][x_pos + j] =
                dot_canvas[y_pos - i][x_pos + j] | V->get()->dot_canvas[V->get()->height - i - 1][j] >> right; // 变换前面的部分
            // // BUG

            if (x_pos + 1 + j > width - 1)
                continue;
            dot_canvas[y_pos - i][x_pos + 1 + j] =
                dot_canvas[y_pos - i][x_pos + 1 + j] | V->get()->dot_canvas[V->get()->height - i - 1][j] << left; // 变换后面的部分
        }
    }
}


void Coord_mono::remake(int x1, int y1, int _width, int _height)
{
    const int y_pos = (flag_coord == world_coord ? height - y1 : y1); //两种坐标系
    const int x_pos = (x1 - 1) / 8; // canvas中的真实位置
    // const int y_pos = p_view->height - y;      // canvas中的真实位置
    ...
    for (int i = 0; i < _height; i++) // 对一个char的8个二进制数进行循环（字符表从上到下）（char从下到上）
    {
        if (y_pos - i > height - 1 || y_pos - i < 0)
            continue;
        ...
        for (int j = 0; j < _width; j++)
        {
            ...
            dot_canvas[y_pos - i][x_pos + j] = 0;
            if (x_pos + 1 + j > width - 1)
                continue;
            dot_canvas[y_pos - i][x_pos + 1 + j] = 0;
        }
    }        
}